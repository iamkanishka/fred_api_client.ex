...
- `FredApiClient.HTTP.Error` exception struct with `code`, `status`, `message` fields
...
- `Cache.invalidate_prefix/1` — delete all keys under a prefix (e.g. `"fred:categories:"`)
  using `Cachex.stream!` + `Cachex.Query.build(output: :key)` for safe ETS iteration
...
- Default: 3 retries with 20 s base delay (20 s → 40 s → 60 s), safely within
  FRED's 60 s rate-limit window
...
- Bypass-based integration tests — no real HTTP calls in the test suite
